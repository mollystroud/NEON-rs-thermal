...
################################################################################
# get bboxes
source("R/bboxes.R") # or, create your own bbox here
...
clean_data <- function(values){
  ...
  values <- values[values$observation > 0,]
  return(values)
}
...
#site <- "BARC"
# call functions
#data <- get_LST(bbox = get(paste0(site, "_bbox")), start_date, end_date)

#vals <- get_vals(get(paste0(site, "_points")), data)

#final <- clean_data(vals)

#write_csv(final, paste0("targets/", site, "/", site, "-targets-rs.csv"))
